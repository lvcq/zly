...
impl Multipart {
    ...
    {
        let (file_key, file_size) = match Self::get_file_size_info(headers) {
            Ok((key, size)) => (key, size),
            Err(err) => {
                return Multipart {
                    error: Some(err),
                    safety: Safety::new(),
    ...
    /// 获取文件大小，存储在自定义header `Upload-Info`中
    fn get_file_size_info(
        header: &HeaderMap,
    ) -> Result<(Option<String>, Option<String>), MultipartError> {
        let mut key: Option<String> = None;
        let mut size: Option<String> = None;
        if let Some(upload_info) = header.get("Upload-Info") {
            if let Ok(info_str) = upload_info.to_str() {
                let res: Vec<&str> = info_str.split(';').collect();
                if res.len() != 2 || res[0].trim() == "" {
                    return Err(MultipartError::Parse(ParseError::Header));
                } else {
                    key = Some(String::from(res[0]));
                    size = Some(String::from(res[1]));
                }
            } else {
                return Err(MultipartError::Parse(ParseError::Header));
            }
...
impl InnerMultipart {
    ...
    fn poll(
        ...
        if self.state == InnerState::Eof {
            ...
            let mut headers = if let Some(mut payload) = self.payload.get_mut(safety) {
                match self.state {
            ...
            self.state = InnerState::Boundary;
            if headers.get(&header::CONTENT_LENGTH).is_none() && self.file_size.is_some() {
                if let Some(content_disposition_str) = headers.get(&header::CONTENT_DISPOSITION) {
                    if let Ok(content_disposition) =
                        ContentDisposition::from_raw(content_disposition_str)
                    {
                        if let Some(key_name) = content_disposition.get_name() {
                            if self.file_key == Some(key_name.to_string()) {
                                headers.insert(
                                    header::CONTENT_LENGTH,
                                    header::HeaderValue::from_str(
                                        &self.file_size.as_ref().unwrap(),
                                    )
                                    .unwrap(),
                                );
                            }
