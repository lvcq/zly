...
impl SessionMiddleware {
    ...
    pub fn get_session(&mut self, zreq: &mut ZRequest, response: &mut Response<Body>) {
        let cookie = zreq.headers().get(COOKIE);
        if cookie.is_some() {
            let coo_value = self
    ...
    pub fn set_session(&mut self, zreq: &mut ZRequest, response: &mut Response<Body>) {
